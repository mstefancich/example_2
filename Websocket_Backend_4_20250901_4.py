...
keywords=["BUTTON_1: ","BUTTON_2: ","FILE_NAME: ","SELECT: ",
          "START_PRINT","GEN_SET_","PUMP_SET_","PRINTER_","RequestWifiList","{\"type\":\"wifi_config\"","CallFromPump","SHUTDOWN_SYSTEM"]
functions=["BUTTON_1_pressed","DIR_command","Receive_file","Select_local_file",
           "Initiate_print","SetGenerator","SetPump","ControlPrinter","scan_wifi_ssids","wifi_config","CallFromPump","shutdown_system"]
...
def GetAvailableFile():
    ...
    if resp.status_code==200: # all ok
        ...
        for each in Q:
            ...
            if each.startswith("AvailablePage"):
                 PAGE=each.split(" ")[1].strip()  
    return(ANS,PAGE) # in case of error returns silently empty array
...
def CheckAndUpdate():
    ...
    ANS,PAGE=GetAvailableFile()
    print("Available remote files: ",ANS)
    X=NeedUpdate(fileNameRoot,ANS)
    if X: # checks for updates of main python code
         DownloadAvailableFile(ANS)
         UpdateSymlink()
        
    print("\nAvailable remote web page file: ",PAGE)
    P=NeedUpdate(pageFileNameRoot,PAGE)
    if P: # checks for updates of main python code
         DownloadAvailableFile(PAGE)
         # here I need to 'sudo cp {PAGE} /var/www/html/index.html'
         subprocess.run(["cp", PAGE, dst], check=True)
...
# moved inside main as it needs to run only if a connection is available
# CheckAndUpdate() # verifies if updates are available and if so gets and updates
