...
# adds matplotlib source folder

try:
    import pprint
except Exception as e:
    print("Error importing pprint: ",e)

try:
    import zipfile
except Exception as e:
    print("Error importing zipfile: ",e)

try:
    from pathlib import Path
except Exception as e:
    print("Error importing pathlib.Path: ",e)
...
try:
    ...
    # test at initial level the functionality of the package
    # debug
    #clearScreen()
    #writeText("Debug mode Display test",pos=(50,20))
    #sleep(1)
except Exception as e:
...
ModuleProblem=False # will change to true any time a problem appears in import
try:
    import local_packages.GerberTranslator_1_4_2_integrated as Translator
except Exception as e:
...
try:
    ...
    print("Error in importing datetime: ",e)
    ModuleProblem=True
#debug Screen Print
# clearScreen()
# writeText("86 Translator and Gcode generator imported",pos=(50,20))
# sleep(1)
...
print("Version 8st September 2025 22:50")
try:
...
### routines for system update necessary when new packages need to be installed
...
try:
    ...
    clearScreen()
    writeText("Problem importing packaging",pos=(50,1))
    a=str(e)
    writeText(a,pos=(90,1))
    ModuleProblem=True
    sleep(5)


def ensure_package(install_name, import_name=None, min_version=None):
    global PackageUpdated
    clearScreen()
    printScreen("checking "+install_name,text_color=0b0000011111100000,pos=(10,20))
    """
    Ensure a package is installed at >= min_version.
    install_name: name used with pip (e.g., 'scikit-image')
    import_name:  module name used in import (e.g., 'skimage'); defaults to install_name with '-' replaced by '_'
    """
    ...
    try:
        ...
        if min_version and current:
            if version.parse(current) < version.parse(min_version):
                PackageUpdated=True
                print(f"{import_name} {current} < {min_version}; upgrading {install_name}…")
                subprocess.check_call([sys.executable, "-m", "pip", "install", f"{install_name}>={min_version}"])
                mod = importlib.import_module(import_name)
                current = getattr(mod, "__version__", None)
        elif min_version and current is None:
            PackageUpdated=True
            # Version not available; force install constraint just in case
            subprocess.check_call([sys.executable, "-m", "pip", "install", f"{install_name}>={min_version}"])
            mod = importlib.import_module(import_name)

        print(f"{import_name} ready (version: {getattr(mod, '__version__', 'unknown')})")
        return mod

    except ImportError:
        PackageUpdated=True
        # Not installed; install with optional constraint
        spec = f"{install_name}>={min_version}" if min_version else install_name
        print(f"{import_name} not found; installing {spec}…")
        subprocess.check_call([sys.executable, "-m", "pip", "install", spec])
        mod = importlib.import_module(import_name)
        print(f"{import_name} installed (version: {getattr(mod, '__version__', 'unknown')})")
        return mod
...
def fixPackages():
    ...
    numpy = ensure_package("numpy", min_version="1.23")
    matplotlib = ensure_package("matplotlib", min_version="3.7")
    skimage =ensure_package("scikit-image",'skimage','0.22')
...
def translateFile(data=""):
    ...
    global verbose
    verbose=1
    # creates the directory structrue if needed
...
def CheckAndUpdate():
    ...
    if Updates[0]: # checks if main python code has been updated
        print(">Updating Python code to: "+response[0])
        UpdateSymlink() # how does this know which file does it need to point to?
    if Updates[1]: # check if main web page has been updated
        print(">Updating web page file: ",response[1])
         # here I need to 'sudo cp {PAGE} /var/www/html/index.html'
        subprocess.run(["cp", response[1], dst], check=True)
    if Updates[2]: # should be a zip file
        extension=response[2].split(".")[-1]
        if(extension=="zip"):
            print(">Decompressing file: ",response[2])
            #todo Decompress
            zip_path = Path("/home/pi/"+response[2]) # target zipfile
...
# fix packages as needed (Moved to main)
# fixPackages()
#writeText("Done: TokenFromOctoprint",text_color=0b1111111111111111,pos=(130,20))
#sleep(5)
